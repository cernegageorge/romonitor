...
var API_ORIGIN = "https://api.worldmonitor.app";
var ROMANIA_FEEDS = [
...
var ROMANIA_BBOX = { lamin: 43.5, lomin: 20.2, lamax: 48.3, lomax: 29.8 };
var digestCache = { value: null, expiresAt: 0 };
var aircraftCache = { value: null, expiresAt: 0 };
...
__name(listRomaniaFeedDigest, "listRomaniaFeedDigest");
function parseOpenSkyStates(states) {
  return states.filter((state) => Array.isArray(state) && state[5] != null && state[6] != null).map((state) => ({
    ...
    observedAt: Number(state[4] || Date.now() / 1e3) * 1e3
  }));
}
__name(parseOpenSkyStates, "parseOpenSkyStates");
...
__name(trackRomaniaAircraft, "trackRomaniaAircraft");
var index_default = {
  async fetch(request) {
