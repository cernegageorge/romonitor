...
export async function initI18n(): Promise<void> {
  ...
  // - wmVariant defaults the Romania build to Romanian while still letting a
  //   query param or an explicit user choice override it.
  ...
  detector.addDetector({
    name: 'wmVariant',
    lookup: () => SITE_VARIANT === 'romania' ? 'ro' : undefined,
    cacheUserLanguage: () => { /* variant default is not a user override */ },
  });
  ...
  await i18next
    ...
    .init({
      ...
      detection: {
        order: ['wmQuery', 'wmExplicit', 'wmVariant', 'navigator'],
